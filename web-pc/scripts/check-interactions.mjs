...
const checks = [
  ...
  ['AI assistant window action buttons are wired', source.app.includes('AiAssistantWindow') && source.aiAssistant.includes('@confirm-action="confirmAction"') && source.aiAssistant.includes('apiClient.assistant.confirmAction') && source.aiAssistant.includes('@send="handleSend"')],
  ...
  ['Window geometry can extend underneath the Dock overlay', source.app.includes('? 12 : windowFrameBottom') && source.window.includes('const bottom = 12')],
  ...
  ['App center opens a backend-backed window', source.app.includes('AppCenterWindow') && source.appCenter.includes('apiClient.appCenter.getApps') && source.appCenter.includes('apiClient.appCenter.getCatalog') && source.appCenter.includes('apiClient.appCenter.preview') && source.appCenter.includes('apiClient.appCenter.confirm')],
  ['App center installs third-party apps via governed iframe windows', source.app.includes('AppFrameWindow') && source.app.includes('openAppFrame') && source.appCenter.includes("emit('open-frame'")],
